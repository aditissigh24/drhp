 ...
 */
import { statusOf, severityOf, isOpenFinding } from './status.mjs';
...
/**
 * Discrepancy Exception Report — `circle_plan.md` §8B.
 *
 * Deliberately *not* the MVC. The MVC is the full record of everything checked
 * (all 953 rows, proof that the work was done). This is the short list of what
 * is actually wrong, ranked worst-first, to hand to counsel or the company.
 * Anything a reviewer has already closed out drops off it.
 */
...
export function buildExceptions(claims, anchors, review = {}) {
  const open = claims
    .filter(isOpenFinding)
    // Already decided by a human is no longer an open exception, but the
    // decision travels with the row so nothing disappears silently.
    .sort((a, b) => severityOf(a).order - severityOf(b).order
      || (anchors[a.id]?.page ?? a.page) - (anchors[b.id]?.page ?? b.page));

  const rows = [EXCEPTION_COLUMNS];
  for (const c of open) {
    const a = anchors[c.id];
    const r = review[c.id] || {};
    rows.push([
